...
document.querySelectorAll('.status-update').forEach(select => {
  select.addEventListener('change', function() {
    ...
    const originalValue = this.options[this.selectedIndex].defaultSelected ? this.options[this.selectedIndex].value : this.getAttribute('data-original');
    ...
    if(confirm(`Change order status to "${newStatus}"?`)) {
      ...
      this.value = originalStatus || this.options[0].value;
    }
  });
