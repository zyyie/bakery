...
// Compute the default (no-coupon) shipping fee first so discounts are accurate.
$cart = $_SESSION['cart'] ?? [];
  ...
  function loadPayPal() {
    ...
    fetch('api/paypal-config.php')
      ...
      .then(cfg => {
        ...
        if (String(cfg.client_id).trim() === 'YOUR_PAYPAL_CLIENT_ID_HERE') {
          console.error('PayPal client_id is placeholder:', cfg);
          btnContainer.innerHTML = '<div class="alert alert-warning mb-0">PayPal is not configured on this server. Set <code>PAYPAL_CLIENT_ID</code> (env) or <code>paypal_client_id</code> in <code>backend/config/secrets.local.php</code>.</div>';
          return;
        }
